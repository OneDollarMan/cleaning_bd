...
def login():
    ...
    if form.validate_on_submit():
        user = r.login_user_safe(form.login.data, hashlib.md5(form.password.data.encode('utf-8')).hexdigest())
        if user:
            if user[5]:
                if pyotp.TOTP(user[6]).verify(form.otp.data):
...
def fa():
    if session.get('loggedin'):
        form = forms.FaForm()
        user = r.get_user(session.get('username'))[0]
        if form.validate_on_submit():
            if pyotp.TOTP(user[6]).verify(form.otp.data):
                if r.toggle_2fa(user[0]):
        ...
        return render_template("2fa.html", title='Двухфакторная аутентификация', enable=user[5], secret=user[6], form=form, url=pyotp.totp.TOTP(user[6]).provisioning_uri(name=session.get('username'), issuer_name='Химчистка'))
    else:
...
def generate():
    if session.get('loggedin'):
        if r.add_secret_key_to_user(session.get('username'), pyotp.random_base32()):
